...
export function HealthLayout({
  ...
  return <div className={cn('min-h-screen bg-gradient-to-br from-slate-50 to-slate-100', className)}>
      {/* Health Dashboard Modal */}
      <AnimatePresence>
        {showHealthDashboard && <motion.div initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} exit={{
        opacity: 0
      }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowHealthDashboard(false)}>
            <motion.div initial={{
          scale: 0.9,
          opacity: 0
        }} animate={{
          ...
          scale: 0.9,
          opacity: 0
        }} className="w-full max-w-7xl max-h-[90vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <HealthDashboard />
            </motion.div>
          </motion.div>}
      </AnimatePresence>
      ...
      {enableQuickAccess && <motion.div initial={{
      x: 300
    }} animate={{
      x: isCollapsed ? 240 : 0
          ...
          <div className="health-card bg-white/95 backdrop-blur-lg rounded-l-2xl shadow-2xl border border-white/20">
            {/* Collapse Button */}
            <Button variant="ghost" size="sm" onClick={() => setIsCollapsed(!isCollapsed)} className={cn("absolute -left-10 top-1/2 -translate-y-1/2", "w-10 h-16 rounded-l-xl bg-white/95 border border-r-0 border-white/20", "hover:bg-orange-50 transition-colors")}>
            ...
            <div className={cn("p-4 w-72 transition-all duration-300", isCollapsed && "opacity-0 pointer-events-none")}>
              ...
              <div className="space-y-3 mb-4">
                {quickStats.map((stat, index) => <motion.div key={stat.label} initial={{
              opacity: 0,
              x: 20
            }} animate={{
              opacity: 1,
              ...
              delay: index * 0.1
                    ...
                    <div className={cn("w-2 h-2 rounded-full", stat.trend === 'up' ? 'bg-green-400' : 'bg-yellow-400')} />
                  </motion.div>)}
              </div>
              ...
              <div className="mb-4">
                ...
                width: `${healthStats.weeklyProgress}%`
              }} transition={{
                duration: 1,
                delay: 0.5
              ...
              {/* Action Button */}
              <Button onClick={() => setShowHealthDashboard(true)} className="w-full health-glow bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white border-0">
      ...
      {/* Health Stats Bar */}
      {showHealthStats && <motion.div initial={{
      y: -100
    }} animate={{
      y: 0
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-gray-700">
                  Idade Metabólica: <strong className="text-red-600 bg-[#1f98bf]">{healthStats.metabolicAge} anos</strong>
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Zap className="w-5 h-5 text-orange-500 bg-slate-800" />
                <span className="text-sm font-medium text-gray-700">
            ...
            <Button variant="outline" size="sm" onClick={() => setShowHealthDashboard(true)} className="nav-btn-top">
              <Activity className="w-4 h-4 mr-2" />
              Health Dashboard
            </Button>
          </div>
